...
from TwinComparison import VideoDemo2
import cv2
...
class exampleQMainWindow (QMainWindow):
    def __init__ (self):
        ...
        self.setWindowTitle('Shot Detect - Twin Comparison')
        self.show()
    ...
    def buttonClicked(self):
        ...
        video = VideoDemo2(fname[0])
        validVideo = video.validVideo()
        self.labelInformation.setText(validVideo['info'])
        if validVideo["flag"] == False:
            return
        cal = video.calcDifferent()
        list_distance = cal["list_distance"]
        list_hist = cal["list_hist"]

        thres = video.getThresholds(list_distance)
        t_s = thres["t_s"]
        t_c = thres["t_c"]

        list_boundary = video.getBoundary(t_c, t_s, list_distance, list_hist)
